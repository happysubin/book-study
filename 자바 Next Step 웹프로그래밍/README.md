...
## part 8. Ajax를 활용해 새로고침 없이 데이터 갱신하기

* 여태는 서버 사이드 렌더링 방식으로 실습을 진행했지만, 이번에는 클라이언트 사이드 렌더링 형식으로 진행
* Jquery를 사용해서 진행함.
* View와 ModelAndView를 사용해서 진행함.
* 우리가 원하지 않는 값이 요청 attribute로 들어가는 것이 싫어 ModelAndView로 리팩토링함.
* 이번장은 상대적으로 수월하게 진행한듯. 추상화는 역시 최고. 선생님은 중복코드를 추상 클래스를 사용해서 많이 제거하시는 듯.
* 시작부터 너무 요구사항에 관한 설계를 고민하지 말고, 일단 초기 설계를 잡고 구현하고 계속 리팩토링하자. 조영호 선생님의 말씀과 동일!

## part 9. 두 번째 양파 껍질을 벗기기 위한 중간 점검

* textarea 태그는 input 처럼 value에 값을 넣는게 아니라 <textarea>${value}</textarea> 이렇게 태그 사이에 넣어야 한다.
* 중복 코드가 발생하면 합성을 사용해서 해결하자는 생각이 제일 우선되어야 한다.
* 좋은 코드는 테스트를 짜기 쉬운 코드다. 테스트가 쉬우려면 연관 관계를 잘 관리해서 모킹을 하기 편리해야 한다.
* 의존성 주입을 사용하면 모킹이 굉장히 편리해진다. 따라서 적절한 의존성 주입을 사용하면 테스트 코드 작성이 수월해진다.
* 웬만하면 객체를 생성하는 책임을 분리하고, 사용하는 책임을 분리하자. 
* 이 방식을 사용하면 인터페이스를 주입하면 DIP, SRP, OCP를 지킬 확률이 굉장히 높아진다. DI를 명심!!!


## part 10. 새로운 MVC 프레임워크 구현을 통한 점진적 개선

* 기초적인 자바 리플렉션 api 복습.
* Reflections와 ReflectionUtils 클래스를 사용해 애노테이션이 붙은 클래스와 메서드를 찾음.
* 리플렉션을 사용해 정말 RequestMapping 기능을 구현함. 꽤 뿌듯하다.
* 과거에 배운 핸들러 매핑, 핸들러 어댑터를 구현함.
* 리팩토링 하는 과정, 즉 LegacyMapping과 HandlerMapping을 공존시키는 과정이 제일 중요하다고 생각한다.
* 확장성을 위해 중간 클래스 어댑터를 도입한 것도 물론 중요하다.
* 역시 의존성이 높아지거나 복잡해지면 중간 클래스를 만들어 문제를 해결할 수 있다. 물론 support 메서드의 개념이 제일 중요했다고 생각한다.
* 중간 클래스 개념보다는 SRP 개념을 도입하기 위해 역할을 나눈 것이 제일 중요했다고 생각.

## part 11. 의존관계 주입을 통한 테스트하기 쉬운 코드 만들기

* 서비스 레이어의 핵심적인 역할은 도메인 객체들이 비즈니스 로직을 구현할 수 있도록 도메인 객체를 조합하거나, 로직 처리르 완료했을 때의 상태 값을 DAO를 활용해 데이터베이스에 영구 저장하는 등의 역할을 한다.
* 객체지향 개발을 하기 위한 좋은 연습은 상태 값을 가지는 도메인 객체에서 값을 꺼내려하지 말고 객체에 메시지를 보내 작업을 위임한다는 생각으로부터 시작할 수 있다.
* 핵심 로직을 도메인 객체에 구현함으로써 로직 구현의 복잡도를 낮추고, 테스트하기 쉬운 코드로 구현할 수 있다.
* 비즈니스 로직 테스트가 무엇보다 중요하다.
* 도메인 객체를 테스트하기 쉬운 코드를 만들려고 고민하다보면 자연스럽게 좀 더 객체지향적인 코드를 구현할 수 있게 된다.
* 재귀함수와 리플렉션을 기반으로 컴포넌트 스캔 기술을 간단히 살펴봤는데 인상 깊었다. 정말 리플렉션은 다양한 API를 제공하는구나.