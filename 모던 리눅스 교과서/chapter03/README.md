...
이식 가능한 스크립트는 다양한 시스템(셸, 리눅스 배포관리 등)에서 실행할 수 있다.
그러나 셸의 유형을 하나로 정하더라도(여기서는 배시) 버전이 다르면 모든 기능이 동일한 방식으로 동작하지 않을 수 있다는 점을 기억하자.
결국에는 얼마나 다양한 환경에서 스크립트를 테스트할 수 있는지가 중요해진다.

스크립트는 어떻게 실행될까? 스크립트는 실제로는 단순한 텍스트 파일이라는 점을 기억하자.
즉 확장자는 중요하지 않지만 .sh가 관례가 사용되는 경우가 많다.
이런 텍스트 파일을 실행 가능한 스크립트로 바꾸며 셸에서 실행할 수 있는 것은 당므 두가지 사항 덕분이다.
...
ShellCheck 프로그램을 사용해 명령과 지침을 올바르게 사용하고 있는지 점검 가능.

스크립트는 저장소에 체크인하기에 앞서 bats를 사용해 테스트하는 편이 좋다.

BASH Automated Testing System의 줄임말 -> bats

이 도구를 사용하면 테스트 케이스를 지정할 수 있는 특수 구문과 함께 배시 스크립트 형태로 테스트 파일을 작성할 수 있다.
